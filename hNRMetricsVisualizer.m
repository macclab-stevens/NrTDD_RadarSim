classdef hNRMetricsVisualizer < handle
    ...
    %   NetworkSimulator            - Network simulator object
    %   GNB                         - Node object of gNB
    %   UEs                         - UEs connected to the given GNB
   ...
   properties(Access = private, Hidden)
       ...
       %UEs UE node objects
       % It is a cell array of node objects of type hNRUE
       UEs
   end
    ...
    methods (Access = public)
        function obj = hNRMetricsVisualizer(param, varargin)
            ...
            for idx = 1:2:numel(varargin)
                if strcmp('NetworkSimulator', varargin{idx})
                    networkSimulator = varargin{idx+1};
                    continue;
                end
            ...
            if ~isempty(networkSimulator) && ~isempty(obj.GNB) && ~isempty(obj.UEs)
                scheduleAction(networkSimulator, @obj.plotLiveMetrics, [], (obj.MetricsStepDuration-(15/param.SCS))*1e-3, obj.MetricsStepDuration*1e-3);
            end
        ...
        function addRLCVisualization(obj, varargin)
            ...
            for ueIdx=1:size(obj.UEInfo, 1)
                ...
                for lcIdx=1:numel(dlLogicalChannels)
                    ...
                    if dlUEOfInterestLCHCount < obj.MaxMetricLinesPerSubPlot && ismember(ueId, obj.UEOfInterestListInfo)
            ...
            for ueIdx=1:size(obj.UEInfo, 1)
                ...
                for lcIdx=1:numel(ulLogicalChannels)
                    ...
                    if ulUEOfInterestLCHCount < obj.MaxMetricLinesPerSubPlot && ~isempty(ueOfInterestIdx)
    ...
    methods(Access = private)
        function plotLiveRLCMetrics(obj)
            ...
            if obj.VisualizationFlag ~= 1 % Downlink
                rlcEntityList = obj.GNB.RLCEntities;
                    for ueIdx = 1:numUEs
                        lchIdx = 1;
        ...
        function plotLiveMACMetrics(obj)
            ...
            if obj.VisualizationFlag ~= 1 % Downlink
                ...
                bufferstatus(:, obj.DownlinkIdx) = getBufferStatus(obj.GNB) ./ 1000; % In KB;
        ...
        function plotLivePhyMetrics(obj)
            ...
            if obj.VisualizationFlag ~= 0 % Uplink
                ulBLERInfo = obj.GNB.PhyEntity.ULBlkErr;
